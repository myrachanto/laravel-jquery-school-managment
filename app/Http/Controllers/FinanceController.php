...
class FinanceController extends Controller
{
      public function __construct(){
	    ...
	    public function index()
	    {
            ...
            return view('/admin/finances/index')->with('finances',$finances);
	    }
	...
	public function fees(){
        $yos = yos::all();
        $invoice = Invoice::all();
		return View::make('admin.finances.fees')
			->with('yos', $yos)
		...
		return View::make('admin.finances.studentfees')
			->with('yos', $yos)
			->with('studs', $studs);
	}	
	public function paymentflow(){
		$student = Student::all();
		return View::make('admin.finances.paymentflow')
	...
	public function individualinvo($name){
		...
		return View::make('admin.finances.individualinvo')
			->with('$amt', $amt)
	...
	public function paymentflow2(Request $request){
		...
		return View::make('admin.finances.paymentflow2')
			->with('$debts', $debts)
			->with('$credit', $credit)
	...
	public function confirm(Request $request){
		...
		return View::make('admin.finances.confirm')
			->with('invoiceno', $invoiceno)
			->with('amt', $amt)
			->with('year', $year)
			->with('term', $term)
	...
	{	
		   ...
		   for ($x = 0; $x < $c; $x++){
					$check = Finance::where('studentid', $request->studentid[$x])
								...
								if($check->count()>0)
							{
								return Redirect::to('/admin/finances/fees')
